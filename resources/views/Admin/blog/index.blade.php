...
@section('content')
    ...
    <div class="page-header page-header-default">
        ...
        <div class="breadcrumb-line">
            <ul class="breadcrumb">
                ...
                <li class="active">{{ trans('app.Blog') }}</li>
            </ul>
        </div>
    </div>
    <!-- /page header -->
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            <h5 class="panel-title col-sm-6 pull-left">{{ trans('app.All Blog') }}</h5>
            <a href="{{ url('admin/add_blog') }}" class="btn bg-teal-400 btn-labeled btn-rounded pull-right" ><b><i class=" icon-plus-circle2"></i></b>{{ trans('app.New') }}</a>
        </div>
            ...
            <tbody>
                @foreach ($all as $blog)
                    <tr>
                        ...
                        <td>{{ $blog->title_ar}}</td>
                        <td>{{ $blog->title_en}}</td>
                        <td>{{ strip_tags($blog->description_ar)}}</td>
                        <td>{{ strip_tags($blog->description_en)}}</td>
                        <td>
                            <img src="{{ url('Admin/images/blog/'.$blog->image) }}" style="width:50px;height:50px;border-radius:50%;"/>
                        </td>
                        <td>
